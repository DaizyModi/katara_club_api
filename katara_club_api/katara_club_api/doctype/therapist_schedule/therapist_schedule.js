...
frappe.ui.form.on('Therapist Schedule', {
	refresh: function(frm) {
		cur_frm.fields_dict["therapist_schedule"].grid.wrapper.find('.grid-add-row').hide();
		cur_frm.fields_dict["therapist_schedule"].grid.add_custom_button(__('Add Time Slots'), () => {
					...
					if(values) {
						...
						function check_overlap_or_add_slot(week_day, cur_time, end_time, add_slots_to_child){
							...
							while(cur_time < end_time) {
								...
								if(to_time <= end_time) {
									if(frm.doc.therapist_schedule){
										frm.doc.therapist_schedule.forEach(function(slot) {
									...
									if(add_to_child && add_slots_to_child){
										frm.add_child('therapist_schedule', {
						...
						frm.refresh_field('therapist_schedule');
						if(slot_added){
							frappe.show_alert({
